...
export default function WeaponsPadSpawner({
  curve,
  playerRefs,
  padCount = 10,
  startT = 0.1,
  ...
  return (
    <>
      {pads.map((pad, index) => (
        <WeaponsPad key={index} position={pad.position} quaternion={pad.quaternion} playerRefs={playerRefs} />
      ))}
    </>
  );
}
