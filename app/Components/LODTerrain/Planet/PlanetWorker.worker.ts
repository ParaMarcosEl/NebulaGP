...
const processBuildChunk = async (
  ...
  const radius = planetSize; // Corrected vertex count calculation: (segments + 1) * (segments + 1)
  const rows = segments + 1;
  const cols = segments + 1;
  const vertexCount = rows * cols; // Helper: rotate a point by quaternion (if orientation supplied)
  const rotateByQuat = (
    px: number,
    py: number,
    pz: number,
    ...
    if (!q) return [px, py, pz]; // quat * v * quat_conj
    const qx = q.x,
      qy = q.y,
      qz = q.z,
      qw = q.w; // t = 2 * cross(q.xyz, v)
    const tx = 2 * (qy * pz - qz * py);
    const ty = 2 * (qz * px - qx * pz);
    const tz = 2 * (qx * py - qy * px); // v' = v + qw * t + cross(q.xyz, t)
  ...
  for (let y = 0; y < rows; y++) {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      dispArr[ix + 1] = py * r;
      dispArr[ix + 2] = pz * r;
      rawElev[y * cols + x] = disp; // Track min/max elevations
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      const tyz = dispArr[down + 2] - dispArr[up + 2]; // normal = cross(tangentX, tangentY)

      let nx = txy * tyz - txz * tyy;
      let ny = txz * tyx - txx * tyz;
      let nz = txx * tyy - txy * tyx; // normalize
