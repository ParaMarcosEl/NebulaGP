...
import { terrainElevationRidged, terrainElevationFBM, FBMParams } from '@/Components/LODTerrain/Planet/Worker/fbm';
...
//   // Precompute displaced positions for central difference
...
//       const disp = params.useRidged ? terrainElevationRidged([basePos.x, basePos.y, basePos.z], params) : terrainElevationFBM([basePos.x, basePos.y, basePos.z], params);
//       const r = radius + disp * (params.uMaxHeight ?? 10);
//       const pos = new Vector3(px * r, py * r, pz * r);
...
//       // Elevation
//       elevations[idx] = rawElevations[y][x];

//       // Tangent vectors using neighbors
//       const prevX = displaced[y][Math.max(x - 1, 0)];
//       const nextX = displaced[y][Math.min(x + 1, cols - 1)];
//       const tangentTheta = nextX.clone().sub(prevX);

//       const prevY = displaced[Math.max(y - 1, 0)][x];
//       const nextY = displaced[Math.min(y + 1, rows - 1)][x];
//       const tangentPhi = nextY.clone().sub(prevY);

//       const normal = tangentTheta.cross(tangentPhi).normalize();
...
//   self.postMessage({ type: 'chunk_ready', posBuffer, normalBuffer, uvBuffer, elevationBuffer });
// };
