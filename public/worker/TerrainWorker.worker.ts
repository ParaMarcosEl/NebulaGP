...
function permuteScalar(x: number): number {
  return ((x * 34.0 + 1.0) * x) % 289.0;
}

// eslint-disable-next-line @typescript-eslint/no-unused-vars
function permuteVec3(x: [number, number, number]): [number, number, number] {
  return [permuteScalar(x[0]), permuteScalar(x[1]), permuteScalar(x[2])];
}

function noise(v: { x: number; y: number }): number {
  const C = { x: 0.211324865, y: 0.366025404, z: 0.577350269, w: 0.0243902439 };
  const dot = (v1: { x: number; y: number }, v2: { x: number; y: number }) =>
    v1.x * v2.x + v1.y * v2.y;
...
function fbm(pos: { x: number; y: number }, params: TerrainChunkRequest['noiseParams']): number {
  let total = 0.0;
...
function getElevation(
  ...
  params: TerrainChunkRequest['noiseParams'],
): number {
...
self.onmessage = (e: MessageEvent) => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const { positionsSAB, uvsSAB, noiseParams, chunkSize, resolution } = e.data;

  const positions = new Float32Array(positionsSAB);
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const uvs = new Float32Array(uvsSAB);
  ...
  self.postMessage(() => {
    ...
    return ({ normals, elevations /* SAB not needed for inputs */ })}
  );
